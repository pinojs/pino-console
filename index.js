...
class Console {
  ...
  // Internal method for logging with level checking and formatting
  _logWithLevel (level, ...args) {
    ...
    // Apply format specifiers if first argument is a string
    if (typeof args[0] === 'string') {
      const formatted = this._formatWithSpecifiers(args[0], args.slice(1))
      ...
      const message = args.map(arg => {
        ...
        return inspect(arg, { colors: false, depth: 2 })
      }).join(' ')
  ...
  // Format specifiers implementation
  _formatWithSpecifiers (template, args) {
    let argIndex = 0
    const result = template.replace(/%[sdioO%]/g, (match) => {
      if (match === '%%') {
        return '%'
      }
      ...
      switch (match) {
        ...
        case '%o':
          return inspect(arg, { colors: false, depth: 1 })
        case '%O':
          return inspect(arg, { colors: false, depth: Infinity })
        default:
          return match
      }
    })
    ...
    if (remainingArgs.length > 0) {
      const formatted = remainingArgs.map(arg =>
        typeof arg === 'string' ? arg : inspect(arg, { colors: false, depth: 2 })
      ).join(' ')
